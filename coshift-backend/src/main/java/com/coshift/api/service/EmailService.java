...
public class EmailService {
    ...
    @Async
    public void sendPasswordResetEmail(String toEmail, String firstname, String code) {
        try {
            MimeMessage message = mailSender.createMimeMessage();
            MimeMessageHelper helper = new MimeMessageHelper(message, true, "UTF-8");
            ...
            helper.setFrom(fromAddress, "CoShift");
            helper.setTo(toEmail);
            helper.setSubject("CoShift - Réinitialisation de votre mot de passe (" + code + ")");
            helper.setText(buildPasswordResetEmailHtml(firstname, code), true);

            mailSender.send(message);
            log.info("Email de réinitialisation envoyé à {}", toEmail);

        } catch (MessagingException | java.io.UnsupportedEncodingException e) {
            log.error("Impossible d'envoyer l'email de réinitialisation à {} : {}",
    ...
    private String buildPasswordResetEmailHtml(String firstname, String code) {
        return """
            <!DOCTYPE html>
            <html lang="fr">
            <head>
            ...
            <body>
              <div class="container">
                <div class="logo">Co<span>Shift</span></div>
                <h2>Bonjour %s,</h2>
                <p>Une réinitialisation de mot de passe a été demandée pour votre compte CoShift. Entrez le code ci-dessous dans l'application pour en choisir un nouveau :</p>
                <div class="code-box">
                  <div class="code">%s</div>
                  <div class="expiry">Ce code expire dans 1 heure</div>
                </div>
                <p class="warn">Si vous n'êtes pas à l'origine de cette demande, ignorez cet email : votre mot de passe actuel reste valable et inchangé.</p>
                <div class="footer">
                  © 2026 CoShift — Plateforme de covoiturage organisationnel<br>
                  Cet email a été envoyé automatiquement, merci de ne pas y répondre.
                </div>
